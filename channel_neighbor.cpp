...
//int				k				= 1000;			// number of nearest neighbors
// test algorithm
int				k				= 100;
// set the number of nearest neighbors as 100
int				dim				= 3;			// dimension
double			eps				= 0;			// error bound
int				maxPts			= 20000;	    // maximum number of data points
...
int main(int argc, char **argv)
{
    ...
    ANNdist sqRad=2.5;                          // fixed radius bound

    nPts = 0;									// read data points
    
    ofstream channel_neighbors;
    ...
    channel_neighbors.open ("channel_neighbors.txt");
    ...
    while (readPt(*queryIn, queryPt)) {		// read query points
		...
		for (int i = 0; i < k; i++) {			// print & write summary
			dists[i] = sqrt(dists[i]);			// unsquare distance
			cout << "\t" << i << "\t" << nnIdx[i] << "\t" << dists[i] << "\n";
            channel_neighbors<<setprecision(7)<< nnIdx[i] << "\t" << dists[i] << "\n";
        }
        
    }
    channel_neighbors.close();
